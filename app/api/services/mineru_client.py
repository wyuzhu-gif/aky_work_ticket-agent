...
class MinerUClient:
    ...
    async def _download_and_parse_zip(self, full_zip_url: str, cache_key: str) -> tuple[Dict[str, Any] | List[Any], Dict[str, Any]]:
        ...
        with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
            ...
            best_score = -1
            best_data: Any = None
            # Track model.json for table content supplementation
            model_data: Any = None
            ...
            for name in candidates:
                try:
                    ...
                    # Save model.json (nested list[list[dict]]) for table supplementation
            ...
            # Supplement empty tables in content_list with model.json table HTML
            if model_data and isinstance(best_data, list):
                model_tables = _extract_model_tables(model_data)
                if model_tables:
                    for item in best_data:
                        if isinstance(item, dict) and item.get("type") == "table":
                            page_idx = item.get("page_idx")
                            if page_idx is not None and not item.get("table_body"):
                                html = model_tables.get(int(page_idx), "")
                                if html:
                                    item["table_body"] = html
...
def _extract_model_tables(model_data: List[List[Dict[str, Any]]]) -> Dict[int, str]:
    """Extract table HTML from model.json (nested page-grouped format), keyed by page_idx."""
    tables: Dict[int, str] = {}
    for page_idx, page_items in enumerate(model_data):
        ...
        for item in page_items:
            ...
            if isinstance(content, dict):
                ...
                if html:
                    tables[page_idx] = html
    return tables


def _html_table_to_text(html: str) -> str:
    """Convert HTML table to readable plain text for LLM consumption."""
    import re
    # Remove all HTML tags, replace </td></tr> etc with separators
    text = re.sub(r'<br\s*/?>', '\n', html)
...
def _paragraphs_from_blocks_list(items: List[Any], *, meta: Dict[str, Any] | None) -> List[Dict[str, Any]]:
    ...
    # Flatten nested page-grouped structure: list[list[dict]] -> list[(page_idx, dict)]
    flat_items: List[tuple[int, dict]] = []
    for outer_idx, item in enumerate(items):
        if isinstance(item, list):
            for sub in item:
                if isinstance(sub, dict):
                    flat_items.append((outer_idx, sub))
        elif isinstance(item, dict):
            flat_items.append((0, item))

    for inferred_page_idx, item in flat_items:
        ...
        # content field can be a string or a dict (content_list_v2 / model.json format)
        html_from_content = ""
        if isinstance(raw_content, dict):
            # Check for HTML table first (most important for tables)
            html_from_content = raw_content.get("html", "")
            # Extract text from nested sub-fields
            parts = []
            for key in ("paragraph_content", "page_header_content", "page_footer_content"):
                sub = raw_content.get(key, [])
        ...
        text = raw_text or str(raw_content) or ""
        text = _fix_mojibake(str(text)).strip()

        # Convert HTML table to readable text (from table_body or content.html)
        table_html = item.get("table_body", "") or html_from_content
        if table_html and item_type == "table":
        ...
        elif item.get("page_num"):
            page_num = int(item["page_num"])
        else:
            page_num = inferred_page_idx + 1
        canvas = None
        if meta and "page_canvas_sizes" in meta:
...
def _score_extraction_json(data: Any) -> int:
    ...
    Handles both flat lists and nested (page-grouped) list[list[dict]] structures.
    """
    try:
        if isinstance(data, list):
            ...
            for it in data[:200]:
                if not isinstance(it, dict):
                    # Nested page-grouped structure: list[list[dict]]
                    if isinstance(it, list):
                        for sub in it[:200]:
                            if isinstance(sub, dict) and ("bbox" in sub or "bounding_box" in sub) and ("text" in sub or "content" in sub):
                                score += 1
                    continue
                if ("bbox" in it or "bounding_box" in it) and ("text" in it or "content" in it):
                    score += 1
            return score
