...
def get_value(key: str, default=None):
    ...
    conn = _get_conn()
    try:
        row = conn.execute("SELECT value FROM sq_config WHERE key=?", (key,)).fetchone()
        if row is None:
            return default
        try:
...
def set_value(key: str, value) -> None:
    ...
    try:
        ...
        conn.execute(
            "INSERT INTO sq_config(key, value, updated_at) VALUES(?, ?, datetime('now','localtime')) "
...
def set_many(kv: dict) -> None:
    ...
    try:
        for k, v in kv.items():
            stored = json.dumps(v) if not isinstance(v, str) else v
            conn.execute(
                "INSERT INTO sq_config(key, value, updated_at) VALUES(?, ?, datetime('now','localtime')) "
                "ON CONFLICT(key) DO UPDATE SET value=excluded.value, updated_at=excluded.updated_at",
                (k, stored),
...
def delete_key(key: str) -> None:
    conn = _get_conn()
    try:
        conn.execute("DELETE FROM sq_config WHERE key=?", (key,))
        conn.commit()
    finally:
        conn.close()
