...
function Review() {
  ...
  const [enabledRuleDocIds, setEnabledRuleDocIds] = useState<string[]>([])
  const [ruleDocsExpanded, setRuleDocsExpanded] = useState(false)
  const [rulesExpanded, setRulesExpanded] = useState(false)
  const [typesExpanded, setTypesExpanded] = useState(false)

  const abortControllerRef = useRef<AbortController>()
  const enabledRuleIdsRef = useRef<string[]>([])
  const enabledRuleDocIdsRef = useRef<string[]>([])
  ...
  useEffect(() => {
    enabledRuleDocIdsRef.current = enabledRuleDocIds
  }, [enabledRuleDocIds])
  ...
  const metrics = useMemo(() => {
    ...
    const high = issues.filter((i) => issueRiskLevel(i.type, (i as any).risk_level) === '高').length
    const medium = issues.filter((i) => issueRiskLevel(i.type, (i as any).risk_level) === '中').length
    const low = issues.filter((i) => issueRiskLevel(i.type, (i as any).risk_level) === '低').length
    const highOpen = issues.filter((i) => issueRiskLevel(i.type, (i as any).risk_level) === '高' && normalizeIssueStatus(i.status as unknown as string) === IssueStatus.NotReviewed).length
  ...
  const runCheck = useCallback((force = false) => {
    ...
    const currentRuleDocIds = enabledRuleDocIdsRef.current
    if (currentRuleDocIds.length > 0) {
      currentRuleDocIds.forEach(id => params.append('rule_doc_ids', id))
    }
    ...
    <div className={classes.layout}>
      ...
      <Card className={`${classes.panel} ${classes.center}`}>
        <div className={classes.toolbar}>
          ...
          <div className={classes.toolbarSection}>
            ...
            {checkInProgress && (
              <Button size="small" appearance="danger" onClick={stopCheck}>
                停止解析
              </Button>
            )}
      ...
      <div className={classes.right}>
        ...
        <div className={classes.accordion}>
          ...
          {docId && (
            <div className={classes.accordionItem}>
              <div className={classes.accordionHeader} onClick={() => setRuleDocsExpanded(!ruleDocsExpanded)}>
                <span style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                  规则文档
                  <Badge appearance="outline" size="small" color="informative">{enabledRuleDocIds.length}</Badge>
                </span>
                <ChevronDown16Regular className={`${classes.accordionIcon} ${ruleDocsExpanded ? classes.accordionIconOpen : ''}`} />
              </div>
              {ruleDocsExpanded && (
                <div className={classes.accordionContent}>
                  <RuleDocumentsPanel
                    docId={docId}
                    enabledRuleDocIds={enabledRuleDocIds}
                    onEnabledRuleDocIdsChange={setEnabledRuleDocIds}
                  />
                </div>
              )}
            </div>
