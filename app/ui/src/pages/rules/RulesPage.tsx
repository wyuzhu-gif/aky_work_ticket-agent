...
import {
  ...
  Spinner, makeStyles, tokens, Textarea, MessageBar, MessageBarBody, Tooltip,
  Input, Field, Dropdown, Option,
} from '@fluentui/react-components'
import {
  Copy16Regular, DeleteRegular, Edit16Regular, EyeRegular,
  Add16Regular, FolderRegular, FolderAddRegular,
} from '@fluentui/react-icons'
import {
  getRules, createRule, updateRule, deleteRule, getFolders, createFolder, updateFolder, deleteFolder as deleteFolderApi, setDefaultFolder,
} from '../../services/api'
...
export default function PromptRules() {
  ...
  const renderRuleCard = (rule: ReviewRule) => (
    <div key={rule.id} className={classes.ruleCard}>
      ...
      <div className={classes.ruleActions}>
        {rule.prompt && <Tooltip content="预览提示词" relationship="label"><Button size="small" appearance="subtle" icon={<EyeRegular />} onClick={() => setPreviewRule(rule)} /></Tooltip>}
        <Tooltip content="复制" relationship="label"><Button size="small" appearance="subtle" icon={<Copy16Regular />} onClick={() => copyToClipboard(rule.prompt || rule.description)} /></Tooltip>
        <Tooltip content="编辑" relationship="label"><Button size="small" appearance="subtle" icon={<Edit16Regular />} onClick={() => openEditRule(rule)} /></Tooltip>
        <Tooltip content="移动到文件夹" relationship="label"><Button size="small" appearance="subtle" icon={<FolderRegular />} onClick={() => { setMoveRuleTarget(rule); setMoveToFolderId(rule.folder_id) }} /></Tooltip>
        <Tooltip content="删除" relationship="label"><Button size="small" appearance="subtle" icon={<DeleteRegular />} onClick={() => setDeleteRuleTarget(rule)} /></Tooltip>
      </div>
    </div>
  )
  ...
  return (
    <div className={classes.container}>
      <Text size={500} weight="semibold">规则库</Text>
