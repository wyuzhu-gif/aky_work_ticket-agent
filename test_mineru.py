...
async def parse_pdf_with_mineru(pdf_path: str, api_key: str) -> list[dict]:
    ...
    async with httpx.AsyncClient(timeout=300) as client:
        ...
        with zipfile.ZipFile(io.BytesIO(zip_bytes)) as zf:
            ...
            print(f"找到的 JSON 文件: {json_files}")
            
            # 保存所有 JSON 文件到输出目录
            output_dir = "./tutorials/data/output"
            ...
            for json_file in json_files:
                ...
                # 保存 JSON 文件
                output_file = os.path.join(output_dir, f"raw_{Path(json_file).name}")
                with open(output_file, "w", encoding="utf-8") as f:
                    json.dump(json_content, f, ensure_ascii=False, indent=2)
                print(f"已保存原始 JSON 文件: {output_file}")
        ...
        if isinstance(content, dict):
            print(f"JSON 顶层键: {list(content.keys())}")
            
            # 检查是否有 pdf_info 字段
            if "pdf_info" in content:
                pdf_info = content["pdf_info"]
                print(f"pdf_info 类型: {type(pdf_info)}")
                if isinstance(pdf_info, list):
                    print(f"pdf_info 长度: {len(pdf_info)}")
                    if pdf_info:
                        first_page = pdf_info[0]
                        print(f"第一页键: {list(first_page.keys())}")
                        
                        # 检查是否有 para_blocks 字段
                        if "para_blocks" in first_page:
                            para_blocks = first_page["para_blocks"]
                            print(f"para_blocks 类型: {type(para_blocks)}")
                            if isinstance(para_blocks, list):
                                print(f"para_blocks 长度: {len(para_blocks)}")
                                if para_blocks:
                                    first_block = para_blocks[0]
                                    print(f"第一个块键: {list(first_block.keys())}")
        elif isinstance(content, list):
            print(f"content 是列表，长度: {len(content)}")
            if content:
                print(f"第一个元素类型: {type(content[0])}")
                if isinstance(content[0], list):
                    print(f"第一个子列表长度: {len(content[0])}")
                    if content[0]:
                        print(f"第一个子列表的第一个元素类型: {type(content[0][0])}")
                        if isinstance(content[0][0], dict):
                            print(f"第一个子列表的第一个元素键: {list(content[0][0].keys())}")
                            # 打印第一个子列表的第一个元素的内容
                            print(f"第一个子列表的第一个元素: {content[0][0]}")
                elif isinstance(content[0], dict):
                    print(f"第一个元素键: {list(content[0].keys())}")
...
async def main():
    ...
    try:
        ...
        json_output = os.path.join(output_dir, f"{pdf_name}_paragraphs.json")
        txt_output = os.path.join(output_dir, f"{pdf_name}_paragraphs.txt")
        
        # 保存为 JSON 文件
        with open(json_output, "w", encoding="utf-8") as f:
            json.dump(paragraphs, f, ensure_ascii=False, indent=2)
        print(f"已保存 JSON 文件: {json_output}")
        
        # 保存为文本文件
        with open(txt_output, "w", encoding="utf-8") as f:
            for i, p in enumerate(paragraphs):
                f.write(f"[{i}] 第{p['page_num']}页:\n")
                f.write(f"{p['content']}\n")
                f.write("-" * 50 + "\n")
        print(f"已保存文本文件: {txt_output}")
