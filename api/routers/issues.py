...
async def get_pdf_issues(
    ...
    rule_doc_ids: Optional[List[str]] = Query(None, description="List of rule document IDs to apply"),
    ...
    try:
        ...
        if rule_doc_ids:
            ...
            logging.info(f"Using {len(rule_documents)} rule documents for review")
        ...
        if stored_issues:
            ...
            issues_stream = issues_service.initiate_review(str(pdf_path), user, date_time, custom_rules, rule_documents)
...
async def start_issue_hitl(
    ...
    interrupt = await issues_service.hitl.start_update(
    ...
    return HitlStartResponse(
        thread_id=thread_id,
        interrupt_id=interrupt.get("id") if isinstance(interrupt, dict) else None,
        proposed_action=proposed_action,
        raw_interrupt=interrupt if isinstance(interrupt, dict) else None,
    )
