...
OUTPUT_FORMAT_PROMPT = """**铁律 (报告数字必须来自 DataFrame, 严禁幻觉编造, 违反将导致回答错误)**:
...
- 严禁: 拿历史对话里其他企业的数字写到当前报告
- 必须: 报告里引用的数字 = DataFrame 实际行数 / 列值, 0 行就明说 0 行, **没数据就明说没数据**
- **严禁编造百分比/比例/分布** — 例如 DataFrame 没有"作业类型"列, 严禁写"100% 包含动火"这种统计; **没有数据支持的结论不要写**
- **严禁编造标准条款** — 引用 GB/AQ 等条款时, 必须从 hermes + llm-wiki 查到的内容来 (即 wiki_enhancement), 严禁自己编条款号
...
输出图表 JSON 后, 紧接着输出专业分析报告. **必须严格按以下 4 段结构** (每段都不可省):

## 📊 核心发现
- 3-5 个关键数据点, 每个都要有具体数值 (从 DataFrame 来, 严禁编造)
- **如果 DataFrame 只有部分列, 不要推测未提供的列 (例如没有"作业类型"列, 严禁写"100% 包含动火")**

## 🔍 详细分析
- 深入分析数据模式、趋势、对比、关联
- **只分析 DataFrame 实际包含的列**, 不要给没数据的列做百分比/分布分析

## ⚠️ 风险提示
- 基于数据分析识别出的具体风险点 (高负荷/集中度/类型缺失/数据异常)
- 每条风险要有 DataFrame 数字支撑, 没数字的风险不要写

## 💡 改进建议
- 基于数据 + 风险提出可执行的具体建议
- 引用相关标准条款 (从 wiki_enhancement 来, 不是自己编)
- **建议要落地, 不要泛泛而谈**
