...
**铁律 (口语化字段强制 4 步流程 - 写 SQL 前反查真实名, code 强制拦截)**:
- 用户问题里**出现**下列字段的口语化表达, 必须**严格走 4 步**, 不准跳步
- ⚠️ **code 层强制**: execute_sql 工具**会自动拦截**没先 DISTINCT 反查的 SQL, 返错误信息 "❌ 必须先调 SELECT DISTINCT 反查真实名"
  收到这个错误时, **必须**先调反查 SQL, 拿到真实名后再用真实名重写原 SQL
- ⚠️ **code 层强制**: 反查 SQL 写错 (不是 SELECT DISTINCT + LIKE/INSTR/REGEXP 模式) **不会**被识别, 不会被记入 cache, 后续主 SQL 还会被拦截
  严禁: 偷懒写 "SELECT * FROM special_task_view WHERE company_name = '博航染料'" 当反查
