...
class Milvus_VectorStore(VannaBase):
    ...
    def __init__(self, config=None):
        ...
        # metric_type 必须与索引一致，否则搜索报错
        # 默认 COSINE (与 MyVanna 默认一致, 避免 L2/COSINE 冲突)
        self.metric_type = config.get("metric_type", "COSINE") if config else "COSINE"
        self._create_collections()
        self.n_results = config.get("n_results", 3)  # 限制 top-3, 避免 LLM 拿到太多 DDL 思维混乱 (之前 10 个 DDL 9836 字符 LLM 拼错 SQL)

    def _create_collections(self):
        self._sql_collection = "vannasql"
        self._ddl_collection = "vannaddl"
        self._doc_collection = "vannadoc"
        self._create_sql_collection(self._sql_collection)
        self._create_ddl_collection(self._ddl_collection)
        self._create_doc_collection(self._doc_collection)
    ...
    def add_question_sql(self, question: str, sql: str, **kwargs) -> str:
        ...
        self.milvus_client.insert(
            collection_name=self._sql_collection,
    ...
    def add_ddl(self, ddl: str, **kwargs) -> str:
        ...
        self.milvus_client.insert(
            collection_name=self._ddl_collection,
    ...
    def get_training_data(self, **kwargs) -> pd.DataFrame:
        # 显式列出字段, 避免 output_fields=["*"] 在某些 pymilvus 版本下漏掉非主键字段
        ...
        df_sql = pd.DataFrame(
            {
                "id": [doc.get("id", "") for doc in sql_data],
                "question": [doc.get("text", "") for doc in sql_data],
                "content": [doc.get("sql", "") for doc in sql_data],
            }
        )
        ...
        ddl_data = self.milvus_client.query(
            collection_name=self._ddl_collection,
            output_fields=["id", "ddl"],
            limit=MAX_LIMIT_SIZE,
        )

        df_ddl = pd.DataFrame(
            {
                "id": [doc.get("id", "") for doc in ddl_data],
                "question": [None for doc in ddl_data],
                "content": [doc.get("ddl", "") for doc in ddl_data],
            }
        )
        ...
        df_doc = pd.DataFrame(
            {
                "id": [doc.get("id", "") for doc in doc_data],
                "question": [None for doc in doc_data],
                "content": [doc.get("doc", "") for doc in doc_data],
            }
        )
    ...
    def get_similar_question_sql(self, question: str, **kwargs) -> list:
        ...
        for doc in res:
            dict_ = {}
            dict_["question"] = doc["entity"]["text"]
            dict_["sql"] = doc["entity"]["sql"]
            list_sql.append(dict_)
        return list_sql
