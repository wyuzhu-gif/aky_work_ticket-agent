...
def extract_key_phrases(text):
    ...
    Example: "动火作业级别 | ○特级 ○一级 ○二级"
      -> ["动火作业级别", "○特级 ○一级 ○二级"]

    Example: "监护人 | 杨越涂 | ... 动火人及作业编号 | ... 杨越涂:141********036 ... 分析人 | 杨越涂"
      -> ["监护人", "杨越涂", "动火人及作业编号", "分析人"]
    """
    ...
    for part in parts:
        ...
        if len(p) < 2:
            continue
        # Skip pure numbers or very generic content
    ...
    try:
        ...
        for pidx in pages_to_try:
            ...
            if rects:
                ...
                doc.close()
                return rects_to_quadpoints(rects, ph), pidx + 1
            ...
            for phrase in phrases:
                ...
                if len(phrase_clean) < 3:
                    continue
                matched = False
                # Try full phrase, then truncate from end
                for length in range(len(phrase_clean), 2, -1):
                    ...
                    if sr:
                        ...
                        break
            if found_rects:
                logger.info(f"[QuadPoints] S4 prefix page {pidx+1}: {len(found_rects)} for {repr(needle[:60])}")
                doc.close()
                return rects_to_quadpoints(found_rects, ph), pidx + 1
...
print("Rewritten highlight_search.py successfully")