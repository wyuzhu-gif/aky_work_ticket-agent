"""
fix_frontend_panels.py
更新 RulesPanel.tsx 支持提示词的编辑/预览/复制。
更新 RuleLibrary.tsx 添加提示词规则 Tab。
"""
...
import { Add16Regular, Copy16Regular, Delete16Regular, Edit16Regular, Eye16Regular } from '@fluentui/react-icons'
...
export function RulesPanel({ docId, enabledRuleIds, onEnabledRulesChange, onRulesCountChange, hideHeader = false }: RulesPanelProps) {
  ...
  function copyPrompt(rule: ReviewRule) {
    const text = rule.prompt || rule.description
            ...
            rules.map(rule => (
              ...
              >
                ...
                <div className={classes.ruleActions}>
                  {rule.prompt && (
                    <Tooltip content="预览提示词" relationship="label">
                      <Button size="small" appearance="subtle" icon={<Eye16Regular />} onClick={() => setPreviewRule(rule)} />
                    </Tooltip>
                  )}
                  <Tooltip content="复制提示词" relationship="label">
                ...
                <div key={rule.id} className={classes.ruleItem}>
                  ...
                  <div className={classes.ruleContent}>
                    <div className={classes.ruleName}>
                      ...
                      {rule.is_preset && <span className={classes.presetBadge} style={{ marginLeft: '8px' }}>预设</span>}
                    </div>
          ...
          <DialogBody>
            ...
            <DialogContent>
              <div className={classes.promptPreview}>
                {previewRule?.prompt || previewRule?.description || '（无提示词）'}
              </div>
            </DialogContent>
            <DialogActions>
              <Button
                ...
                icon={<Copy16Regular />}
                onClick={() => copyPrompt(previewRule!)}
              >
                复制
              </Button>
          ...
          <DialogBody>
            ...
            <DialogContent>
              ...
              <Field label="提示词正文" className={classes.formField}
                hint="完整的审核提示词，会在审核时注入到 LLM。留空则仅使用名称和描述。">
                <Textarea
                  value={formPrompt}
                  onChange={(_, data) => setFormPrompt(data.value)}
                  placeholder="例如：检查文档中是否存在XXX问题，重点关注..."
                  rows={5}
                />
              </Field>
